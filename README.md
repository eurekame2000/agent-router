...
## 难度路由（v1.5.0：投机起步 + 事后异步评估）

| 档位 | 模型 | 场景 |
|---|---|---|
| cheap | `deepseek-v4-flash:0731` | 默认起步（投机路由，零预判延迟） |
| medium | `kimi-k2.7-code` | 常规任务（写代码 / 总结） |
| smart | `glm-5.3-flash` | 复杂任务（重构 / 调试 / 安全审计 / 长输入） |

工作方式（v1.5.0 起替代旧的请求前规则打分）：

- **投机起步**：新会话/未知会话直接走 cheap，先服务后评估，零预判延迟
- **事后异步评估**：响应完成后 judge 异步评「请求 + 已完成回答」，绝不阻塞用户流
- **升级锁存（latch）**：连续 2 次判定「吃力」→ 该会话锁存高档位；活跃期读即续期，闲置 2h 自动过期回落 cheap
- **显式档位不受影响**：请求 `model` 指定 flash/pro/smart 时固定路由
